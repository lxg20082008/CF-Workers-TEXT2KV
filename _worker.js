...
function 下载bat(域名,token) {
	return [
	  ...
	  `setlocal enabledelayedexpansion`, 
	         ``,
	         `rem 获取默认浏览器的路径`, 
	         `for /f "tokens=2*" %%a in ('reg query "HKCR\\http\\shell\\open\\command" /ve ^| find "REG_SZ"') do (`, 
	         `    set BROWSER_PATH=%%b`, 
	         `)`, 
	         `rem 去除路径两侧的引号`, 
	         `set BROWSER_PATH=!BROWSER_PATH:"=!`, 
	         ``,
	         `rem 如果找到默认浏览器路径，则直接使用默认浏览器打开 URL`, 
	         `if not "!BROWSER_PATH!"=="" (`, 
	         `    start "" !BROWSER_PATH! %URL%`, 
	         `) else (`, 
	         `    rem 如果未找到默认浏览器路径，则按顺序启动 Chrome 和 Edge 浏览器`, 
	         `    start chrome %URL%`, 
	         `    if not errorlevel 1 (`, 
	         `        exit`, 
	         `    ) else (`, 
	         ...
	         `            exit`, 
	         `        ) else (`, 
	         `            rem 如果启动 Chrome 和 Edge 浏览器都失败了，则询问用户输入浏览器的启动命令`, 
	         `            :input_browser_command`, 
	         `            set /p BROWSER_COMMAND=请输入浏览器的启动命令：`, 
	         `            if "!BROWSER_COMMAND!"=="" (`, 
	         `                echo 请输入有效的浏览器命令！`, 
	         `                goto :input_browser_command`, 
	         `            )`, 
	         `            start "" !BROWSER_COMMAND! %URL%`, 
	         `        )`, 
	         `    )`, 
	         `)`, 
	  `endlocal`,
